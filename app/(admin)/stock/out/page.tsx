...
type FormState = {
  date: string;
  reason: "SALE" | "WASTAGE" | "ADJUSTMENT";
  note: string;
  refType: string;
  refId: string;
  lines: Line[];
...
export default function StockOutPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const data = (await apiPost("/stock/out", {
        date: form.date,
        reason: form.reason,
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
          ...
          <label>
            ...
            <select
              ...
              onChange={(event) =>
                update(
                  "reason",
                  event.target.value as "SALE" | "WASTAGE" | "ADJUSTMENT"
                )
              }
            >
        ...
        {movementId ? <p>Created movement: {movementId}</p> : null}
